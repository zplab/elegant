...
class Worm(object):
    ...
    def make_smoothing_filter(filter_type, window_length=24,**filter_params):
        '''Generates a *causal* smoothing filter (i.e. one that only smooths over past values in data)
            ...
            Parameters
                ...
                window_length - (optional) total time over which to perform smoothing
                **filter_params - (optional) kw args to supply for filters (i.e. sigma
        ...
        def gaussian_filter(t, sigma):
            return 1/numpy.exp(-t**2/(2*sigma)).sum() * numpy.exp(-t**2/(2*sigma))
        def uniform_filter(t):
            return numpy.ones_like(t)/len(t)

        def factory(time, data):
            if filter_type == 'gaussian':
                selected_filter = lambda t: gaussian_filter(t, filter_params['sigma'])
            elif filter_type == 'uniform';
                selected_filter = uniform_filter

            smoothing_mask = (time >= 0) & (time <= window_length)
            return numpy.dot(selected_filter(time[smoothing_mask]) * data[smoothing_mask])
        return factory


    def smooth_feature(self, feature, smoothing_filter, age_feature='age', min_age=-numpy.inf, max_age=numpy.inf):
        """Smooth a feature with a prescribed filter

            Parameters
                feature - feature to filter over
                smoothing_filter - filter function to apply to the feature data; the
                    filter should have the call signature: smoothing_filter(time, data)
                    where time has been centered so that t=0 corresponds to the
                    timepoint where smoothing is performed for and positive time
                    corresponds to the past; this filter is generated by make_smoothing_filter
                age_feature - Specified age_feature to use with get_time_range
        ...
        for present_age in ages:
            ...
            smoothed_values = numpy.append(smoothed_values,
                smoothing_filter(centered_ages, data))

        age_mask = numpy.isin(getattr(self.td, age_feature), ages)
        smoothed_feature = numpy.empty_like(age_mask)
        smoothed_feature[age_mask] = smoothed_values
        smoothed_feature[~age_mask] = getattr(self.td, feature)[~age_mask]
