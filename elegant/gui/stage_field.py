...
class StageField(annotator.AnnotationField):
    ...
    def __init__(self, name='stage', stages=['egg', 'larva', 'adult', 'dead'], transitions=['hatch', 'adult', 'dead'], shortcuts=None):
        ...
        If experiment_age annotations are already present (i.e. as generated by
    ...
    def update_ages(self):
        ...
        dead_t = None
        for page in self.flipbook.pages:
            if not hasattr(page, 'annotations') or 'experiment_age' not in page.annotations:
                return
            experiment_age = page.annotations['experiment_age']
            page_stage = self.get_annotation(page)
            if hatch_t is None and page_stage != self.stages[0]: # any stage afater the first is hatching, whatever it is
                hatch_t = experiment_age
            elif dead_t is None and page_stage == self.stages[-1]:
                dead_t = experiment_age

        for page in self.flipbook.pages:
            experiment_age = page.annotations['experiment_age']
            if hatch_t is not None:
                page.annotations['age'] = experiment_age - hatch_t
            if dead_t is not None:
                page.annotations['ghost_age'] = experiment_age - dead_t